...
import React, { useEffect, useState, useRef, FC, createRef, forwardRef, useImperativeHandle } from "react";
...
import { OG_win_variants, whitelist_win_variants } from "../../../win_variants";
...
import ticket2_4 from "../../../media/whitelist.jpg";

const Og_lootbox_2: FC<Props> = ({ onChange, setColor, setActiveMenuElement, picture, price, title, URLtitle, colors, text, win_variants, lootboxAddress }) => {
  ...
  function initWheel2() {
    ...
    for (var x = 0; x < 5; x++) {
      ...
      //   }
      for (let i = 0; i < chengedMass.length; i++) {
        if (chengedMass[i].text === "Free Mint") {
          row += `  <div class='element2' id='${chengedMass[i].text}'><img src=${ticket2_1} alt="" class="imageLootbeers freeMint" style="height: 0"/><\/div>`;
        } else if (chengedMass[i].text === "OG") {
          row += `  <div class='element2' id='${chengedMass[i].text}'><img src=${ticket2_2} alt="" class="imageLootbeers freeMint" style="height: 0"/><\/div>`;
        } else if (chengedMass[i].text === "NOTHING") {
          row += `  <div class='element2' id='${chengedMass[i].text}'><img src=${ticket2_3} alt="" class="imageLootbeers freeMint" style="height: 0"/><\/div>`;
        } else {
          row += `  <div class='element2' id='${chengedMass[i].text}'><img src=${ticket2_4} alt="" class="imageLootbeers freeMint" style="height: 0"/><\/div>`;
        }
      }
      row += "</div>";
  ...
  const spinClick = async (ticket: Boolean, ticketData: PublicKey | undefined) => {
    try {
      ...
      const ArrayRandom: Array<number> = [0, 1, 2, 3];
      // let rendomNum2 = await getRandomArrayElement(ArrayRandom); // тут res.data.id
      // let rendomNum2 = iTicket; // тут res.data.id
      let rendomNum2 = getRandomArrayElement(ArrayRandom); // тут res.data.id
      //   let rendomNum2 = 1; // тут res.data.id
      // console.log(whitelist_win_variants[rendomNum2].text);
      settextState_About(whitelist_win_variants[rendomNum2].text);
      setImageStateCong(rendomNum2);

      let rendomNum: number = 0;

      let arr2 = ALLrandom_main_lootboxNumbers;

      // console.log(massStatusreturn);
      // console.log(arr2)
      // let n = arr2.length;
      // let k = Number((arr2.length / 2).toFixed(0));
      // splitArr(arr2, k, n);
      // printArray(arr2, n);
          ...
          <div className="AnimationLootbeers_background_block">
            ...
            <div className="AnimationLootbeers_background_block_text">
              <div className="AnimationLootbeers_background_block_text_mini">
                {imageStatecong === 0 ? <img src={ticket2_3} alt="" className="" style={{ height: "140px", marginBottom: "20px" }} /> : imageStatecong === 1 ? <img src={ticket2_1} alt="" className="" style={{ height: "140px", marginBottom: "20px" }} /> : imageStatecong === 2 ? <img src={ticket2_2} alt="" className="" style={{ height: "140px", marginBottom: "20px" }} /> : <img src={ticket2_4} alt="" className="" style={{ height: "140px", marginBottom: "20px" }} />}
                <br />
                <br />
                <div className="header_a_block">{textState_About}</div>
              </div>
              <div className="buttons_AnimationLootbeers">
                {textState === whitelist_win_variants[1].text || textState === whitelist_win_variants[0].text ? (
                ...
                {textState === whitelist_win_variants[0].text ? (
                ...
                ) : textState === whitelist_win_variants[1].text ? (
                ...
                ) : textState === whitelist_win_variants[2].text ? (
